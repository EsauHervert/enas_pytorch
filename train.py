...
## Here we train the CNN which is sampled from the Controller
def train_shared_cnn(epoch,
    ...
    ## When we are searching for an architecture or evaluating a fixed one.
    ...
    ## This is where we start the training on the network using the images.
    for i, (images, labels) in enumerate(train_loader):
        ...
        if fixed_arc is None:
            ...
            sample_arc = controller.sample_arc ## This is the network that is generated by the Controller.
        else:
            sample_arc = fixed_arc ## This is the fixed network.

        ## Calculate the output of the network and perform gradient step using Cross-entropy Loss.
...
## This is for training the controller network, which we do once we have gone through the exploration of the child architectures.
